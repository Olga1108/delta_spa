...
export const SnakeIntroCard = ({ description, locale, variant }: SnakeIntroCardProps) => {
  ...
  return (
    <article
      data-anim='title'
      className='row-span-6 flex h-full flex-col justify-between overflow-hidden rounded-[10px] bg-[linear-gradient(64.6deg,#220032_17.61%,#560080_57.18%,#9500DC_88.56%)] p-6 shadow-[0_10px_28px_rgba(0,0,0,0.32)]'
    >
      <p className={snakeCardDescriptionClassName}>
        {renderWordsWithHighlight(description, locale)}
      </p>
      <img
        src={heroSnake}
        alt='Purple snake illustration'
        className='pointer-events-none mt-4 h-[22rem] w-full self-center select-none object-contain object-bottom lg:h-[19rem] xl:h-[25rem]'
        draggable={false}
      />
    </article>
  )
